...
function createEvent() {
 ...
 async function ListEvents(user) {
  await fetch(`http://localhost:3000/events/${user.id}`).then(response=> response.json()).then(result => events = result);
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      tableRows += `
        <tr>
          ...
          <td>${event.local}</td>
          <td>
            <img src="./edit.png" width="24" onclick="OpenModelEdit(${JSON.stringify(event).replace(/"/g, '&quot;')})"/>
          </td>
        </tr>
      `;
    }
...
function UpdateEvent(){
  ...
  let data = JSON.stringify({ date, talks, dist, local, name, id: currentEvent.id });
...
function OpenModelEdit(event) {
  ...
  modal.innerHTML = `
      <form>
          ...
          <input id="event_talks_edit"  value=${event.talks} />

          <label>Públicos Alvo</label>
          <input id="event_dist_edit"  value=${event.dist} />
